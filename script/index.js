...
import { cardTemplate, cards, modalLookPhoto, modalProfile, modalAddCard, editProfileButton, newCardButton,
    saveProfile, saveCard, submitButton, closeModalProfile, closeModalAddCard, closeModalLook, formsObj, modalTitle, modalPlace,
    modalName, modalProfession, profileName, profileProfession, modalPhoto, modalSignature } from './data.js';
...
export function openPhoto(photoCard, nameText) { //вешаем слушателя на фото, получем параметры и сразу пишем в модалку
...
editProfileButton.addEventListener("click", function (event) {
    ...
    const formValidator = new FormValidator(formsObj, modalProfile);
    // formValidator._hideError();
...
newCardButton.addEventListener("click", function () {
    ...
    const formValidator = new FormValidator(formsObj, modalAddCard);
    formValidator.enableValidation();
    formValidator.enterDisable(event);
    saveCard.reset();
});
