export class Api {
  ...
  setLikes(cardId) {
    return fetch(`${this._serverUrl}/v1/${this._cohort}/cards/likes/${cardId}`, {
      method: "PUT",
      headers: {
        authorization: this._token,
      },
      ...
      return Promise.reject(`Ошибка! Лайк не поставлен: ${res.status}, ${res.statusText}`);
    });
  }
  ...
  delLikes(cardId) {
    return fetch(`${this._serverUrl}/v1/${this._cohort}/cards/likes/${cardId}`, {
      ...
      return Promise.reject(`Ошибка! Лайк не снят: ${res.status}, ${res.statusText}`);
    });
  }
  setAvatar(avatarURL) {
    return fetch(`${this._serverUrl}/v1/${this._cohort}/users/me/avatar`, {
      ...
      return Promise.reject(`Ошибка! Аватар не обновлён: ${res.status}, ${res.statusText}`);
    });
  }
}
